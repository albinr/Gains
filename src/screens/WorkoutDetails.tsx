...
import { Button, Headline, IconButton, List } from 'react-native-paper';
...
const Stepper: React.FC<{ minValue?: number, value: number, onValueUpdated: React.Dispatch<React.SetStateAction<number>> }> = ({ onValueUpdated, value, minValue = 0 }) => {
  ...
  return <View style={{flexDirection: 'row' }}>
    ...
    <Text style={{ width: 50, fontSize: 30, fontVariant: ['tabular-nums'], textAlign: 'center', alignSelf: 'center' }}>{ value.toString() }</Text>
...
export default function ModalScreen({ navigation, route:{params: {workout}}}: RootStackScreenProps<'Modal'>) {
  ...
  const setsPerDay = React.useMemo(() => sets.reduce<{title: string, data: Set[]}[]>((acc, set) => {
...
/>
<VictoryAxis dependentAxis crossAxis domainPadding={{ x: 10, y: 10 }} orientation='left'></VictoryAxis>
<VictoryAxis tickFormat={() => ''}  domainPadding={{ x: 10, y: 10 }} orientation='bottom'></VictoryAxis>
</VictoryChart>

      <SectionList sections={setsPerDay} style={{width: '100%'}}
      renderSectionHeader={({ section: { title } }) => (
        <Headline style={{ padding: 10 }}>{title}</Headline>
      )}
      renderItem={({item}) => <List.Item title={<Text>{ item.reps + ' reps @ '}<Text style={{ fontWeight: 'bold' }}>{item.weight + ' kg'}</Text></Text>} description={dayjs(item.createdAt).format('hh:mm')}></List.Item>} />
    </View>
  );
}
